...
} from '@places/src/services/locations.service';
...
export const getNearbyPlaces = async (
  ...
  try {
    ...
    logger.error('GetNearbyPlaces Error:', { details: message });
    res
      .status(500)
      .json({ error: 'Failed to fetch nearby places', details: message });
  }
};

export const getPlacesByIP = async (
  req: express.Request,
  res: express.Response,
): Promise<void> => {
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'Failed to get places by IP';
    logger.error('GetPlacesByIP Error:', { details: message });
    res.status(500).json({ error: message });
  }
};

// 📌 3️⃣ Get Places by Address (Convert Address to Lat/Lng)
export const getPlacesByAddress = async (
  req: express.Request,
  res: express.Response,
): Promise<void> => {
  try {
    const { street, city, state, country, postalCode, query, language } =
      req.body;
    ...
    const message =
      error instanceof Error
        ? error.message
        : 'Failed to get places by address';
    logger.error('GetPlacesByAddress Error:', { details: message });
    res.status(500).json({ error: message });
  }
};
